  ...
  public void updateOutput() {
    ...
    try {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        // Use the String.format() method to format the columns with padding
        String formattedLine = String.format("%-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s",
          columns[0], columns[1], columns[2], columns[3], columns[4], columns[5], columns[6], columns[7], columns[8], columns[9], columns[10]);

        // Append the formatted line to the text area
